...
if ($stavka==0){
    ...
    $redis->select(2);
    $block_ip=$redis->get($ploshadka_id.':'.$_SERVER['REMOTE_ADDR']);
    if ($block_ip) {
        $redis->set($ploshadka_id.':'.$_SERVER['REMOTE_ADDR'], 1, 1296000);
        exit;
    }
    $redis->set($ploshadka_id.':'.$_SERVER['REMOTE_ADDR'], 1, 86400);
    $redis->close();
    ...
    $sql = "UPDATE `balans_ploshadki` SET `balans` = `balans` + ".$stavka.", `".$_GET['t']."`=".$_GET['t']."+1, `".$_GET['t']."_balans`=".$_GET['t']."_balans+".$stavka."  WHERE `date`=CURDATE() AND `ploshadka_id`='".$ploshadka_id."'";
    if (!$dbstat->exec($sql)){
        $sql = "INSERT INTO `balans_ploshadki` SET `ploshadka_id` = '".$ploshadka_id."', `date` = CURDATE(), `balans` = `balans` + ".$stavka.", `".$_GET['t']."`=".$_GET['t']."+1, `".$_GET['t']."_balans`=".$_GET['t']."_balans+".$stavka;
        $dbstat->query($sql);
    }

...
class WidgetcssController
{
    ...
    public static function UpdateCSSfile($id){
        ...
        if ($result['status']){
            $sql="SELECT `forcibly`,`selector`,`selector-title`,`scroll2site_activ`,`scroll2site_fon_color`,`scroll2site_text_size`,`scroll2site_text_font`,`scroll2site_text_color`,scroll2site_perehod_color,`marker_reklamiy`,`css`,`adblock-css` FROM `style_promo` WHERE `id`='".$id."'";
            $css2=$GLOBALS['db']->query($sql)->fetch(PDO::FETCH_ASSOC);
            if ($css2==false){
                $sql="SELECT `forcibly`,`selector`,`selector-title`,`scroll2site_activ`,`scroll2site_fon_color`,`scroll2site_text_size`,`scroll2site_text_font`,`scroll2site_text_color`,scroll2site_perehod_color,`marker_reklamiy`,`css`,`adblock-css` FROM `style_promo` WHERE `id`='0';";
                $css2=$GLOBALS['db']->query($sql)->fetch(PDO::FETCH_ASSOC);
            }
